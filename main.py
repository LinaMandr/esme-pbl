...
def evaluation(board, player=1):
    """evaluation function: +1000 if the AI won, -1000 if the opponent won"""
...
def minimax(board, level, is_max, player=1, level_max=None):
    ...
    if is_max:
        ...
        for square in available:
            ...
            score = minimax(board, level + 1, False, player, level_max)
        ...
        for square in available:
            ...
            score = minimax(board, level + 1, True, player, level_max)
            board[square] = 0        #we cancel the play
...
def best_play(board, player=1, level_max=None):
    ...
    for square in squares_available(board):
        board[square] = player  #the AI plays
        score = minimax(board, 1, False, player, level_max)
...
current_player = 'X'
win = False
ai_symbol_var = tk.StringVar(value="X")
...
def check_win_and_tie():
    ...
    if check_winner(internal_board):   # 'O' won
        print_winner()
        return True
    if check_winner(internal_board,-1):    # 'X' won
        print_winner()
        return True
...
def ai_move(difficulty):
    ...
    if difficulty == "EASY":
        ...
        chosen_index, _ = best_play(internal_board, player, level_max=3)

    else:
        # HARD: full minimax, unbeatable
        chosen_index, _ = best_play(internal_board, player)

    # Convert flat index back to (row, col) in col-major storage
    col = chosen_index // 3
    row = chosen_index % 3

    buttons[col][row].config(text=ai_symbol, fg="#0f3460")
    ...
    # Trigger AI_2's next move in 'AIvsAI' mode
    if game_mode_var.get()=="AIvsAI":
        root.after(300, lambda: ai_move(difficulty_var.get()))
...
def restart_game():
    ...
    if game_mode_var.get()=='AIvsAI':
        ...
        current_player = "X" if ai_symbol == 'O' else current_player = 'O'
